...
def sortClinetTime():
    global low_client,median_client,high_client,client_time
...
def initiate_sampler_query(queue, numOfClients,dict_users_unlabeled):
    # Initiate the clientSampler
    global distance_dict,client_time,low_client,median_client,high_client
...
def run(queue, param_q, stop_signal, clientSampler,dataset_train,test_dataset,dict_users_labeled):
    global logDir, sampledClientSet,client_time,low_client,median_client,high_client,epoch_time
            ...
            try:
                ...
                for i, (clientId, distance) in enumerate(zip(clientIds, distance_list)):
                    ...
                    if repeat == 3:
                        ...
                        if clientId in low_client.keys():
                            low_client[clientId] = distance
                        elif clientId in median_client.keys():
                            median_client[clientId] = distance
                        else:
                            high_client[clientId] = distance
                    else:
                        if clientId in low_client.keys():
                            low_client[clientId] = distance
                            repeatClient.append(clientId)
...
def getTop(numToSample):
    global low_client, median_client, high_client, epoch_time, client_time
    if args.data_set == "cifar10":
        low_number = 40
        median_number = 7
        high_number = 3
    ...
    low_cls = sorted(low_client, key=low_client.get, reverse=True)[:low_number]
    median_cls = sorted(median_client, key=median_client.get, reverse=True)[:median_number]
    high_cls = sorted(high_client, key=high_client.get, reverse=True)[:high_number]
    
    return_list = low_cls + median_cls + high_cls
