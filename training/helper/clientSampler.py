...
class clientSampler(object):
    ...
    def registerClient(self, hostId, clientId, dis, size, speed=[1.0, 1.0], duration=1):
        ...
        if size >= self.filter_less and size <= self.filter_more:
            # size: the number of image in client
            # clientId: 100
            # duration:1
    ...
    def resampleClients(self, numOfClients, cur_time=0,have_oort = False):
        #using oort or random to resample the clients
        logging.info(f"path:{oort.__file__}")
        self.count += 1
        logging.info("ccc")
        ...
        # logging.info(f"self.ucbSampler.checkClients(feasible_clients_set): {self.ucbSampler.checkClients(feasible_clients_set)}")
        # logging.info(f"feasible_clients_set: {feasible_clients_set}")
        # logging.info(f"self.count: {self.count}")
        if have_oort:
            logging.info("start use oort {}".format(numOfClients))
            ...
            if self.mode == "oort" and self.count > 1 and self.ucbSampler.checkClients(feasible_clients_set) == True:
                ...
                logging.info("random {}".format(numOfClients))
