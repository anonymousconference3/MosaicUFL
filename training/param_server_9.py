...
def registerTime(total_imgs, numOfClients,dict_users_unlabeled):
    if os.path.exists("/home/your_path/code/FLPerf/benchmark/dataset/data/device_info/client_device_capacity"):
        with open("/home/your_path/code/FLPerf/benchmark/dataset/data/device_info/client_device_capacity", 'rb') as fin:
...
def sortClinetTime():
    global one_client,two_client,three_client,four_client,five_client,six_client,seven_client,eight_client,nine_client,client_time
    ...
    if args.data_set == "cifar10":
        ...
        for each in range(10):
            three_client[sorted_clients[each+62]] = 0
        for each in range(8):
            four_client[sorted_clients[each+72]] = 0
        for each in range(8):
            five_client[sorted_clients[each+80]] = 0
        for each in range(4):
            six_client[sorted_clients[each+88]] = 0
        for each in range(4):
            seven_client[sorted_clients[each+92]] = 0
        for each in range(2):
            eight_client[sorted_clients[each+96]] = 0
        for each in range(2):
            nine_client[sorted_clients[each+98]] = 0
...
def initiate_sampler_query(queue, numOfClients,dict_users_unlabeled):
    # Initiate the clientSampler
    global distance_dict,client_time,one_client,two_client,three_client,four_client,five_client,six_client,seven_client,eight_client,nine_client
...
def run(queue, param_q, stop_signal, clientSampler,dataset_train,test_dataset,dict_users_labeled):
    global logDir, sampledClientSet,client_time,one_client,two_client,three_client,four_client,five_client,six_client,seven_client,eight_client,nine_client,epoch_time
            ...
            try:
                ...
                for i, (clientId, distance) in enumerate(zip(clientIds, distance_list)):
                    ...
                    if repeat == 7:
                        ...
                        elif clientId in seven_client.keys():
                            seven_client[clientId] = distance
                        elif clientId in eight_client.keys():
                            eight_client[clientId] = distance
                        else:
                            nine_client[clientId] = distance
                    else:
                        if clientId in one_client.keys():
                            one_client[clientId] = distance
                            repeatClient.append(clientId)
                            ...
                            if repeat == 8:
                                if w_glob_list[i] != None:
                                    tmp_save[8].append(w_glob_list[i])
                                continue
                            repeatClient.append(clientId)
                        else:
                            nine_client[clientId] = distance
                            if w_glob_list[i] != None:
                                tmp_save[9].append(w_glob_list[i])
                            continue
...
def getTop(numToSample):
    global one_client,two_client,three_client,four_client,five_client,six_client,seven_client,eight_client,nine_client, epoch_time, client_time
    if args.data_set == "cifar10":
        one_number = 25
        two_number = 6
        three_number = 5
    ...
    eight_cls = sorted(eight_client, key=eight_client.get, reverse=True)[:eight_number]
    nine_cls = sorted(nine_client, key=nine_client.get, reverse=True)[:nine_number]
    
    return_list = one_cls + two_cls + three_cls + four_cls + five_cls + six_cls + seven_cls+eight_cls+nine_cls
