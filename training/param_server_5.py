...
def sortClinetTime():
    global one_client,two_client,three_client,four_client,five_client,client_time
    ...
    if args.data_set == "cifar10":
        ...
        for each in range(14):
            three_client[sorted_clients[each+74]] = 0
        for each in range(8):
            four_client[sorted_clients[each+88]] = 0
        for each in range(4):
            five_client[sorted_clients[each+96]] = 0
...
def initiate_sampler_query(queue, numOfClients,dict_users_unlabeled):
    # Initiate the clientSampler
    global distance_dict,client_time,one_client,two_client,three_client,four_client,five_client
...
def run(queue, param_q, stop_signal, clientSampler,dataset_train,test_dataset,dict_users_labeled):
    global logDir, sampledClientSet,client_time,one_client,two_client,three_client,four_client,five_client,epoch_time
...
def getTop(numToSample):
    global one_client,two_client,three_client,four_client,five_client, epoch_time, client_time
    if args.data_set == "cifar10":
        one_number = 25
        two_number = 12
        three_number = 7
    ...
    return_list = one_cls + two_cls + three_cls + four_cls + five_cls
